---
title: "Example: The Hard Brake — PreToolUse Hook Enforcement"
sidebarTitle: "Enforcement Hook"
description: "How the PreToolUse hook intercepts tool calls and returns allow/ask/deny decisions, with CI headless mode auto-denying hot actions."
---

The [Green-Light Brake example](/examples/green-light-brake) shows the **soft brake** — the agent *reasoning* its way to a stop. This example shows the **hard brake**: the `PreToolUse` hook (`enforce-policy.cjs`) intercepting a tool call and stopping it *regardless* of what the model decided. Two layers, defense in depth. The difference matters in one sentence: the soft layer depends on the model choosing to comply; the hard layer doesn't.
...
```json Push to origin — hot, stops for approval
// Input
...
```json Run the approved build — standing approval passes it through
// Input
{"tool_name": "Bash", "tool_input": {"command": "npm run build"}}

// Output
{"permissionDecision": "allow", "reason": "keel: covered by standing approval (npm run build)"}
```

```json Edit client-facing code — hot path, stops for approval
// Input
{"tool_name": "Write", "tool_input": {"file_path": "src/app/page.tsx"}}
...
The skill reasons; the hook backstops. See [SPEC.md §8.1](/reference/spec#8-conformance) for the enforcing-implementation conformance requirements.
